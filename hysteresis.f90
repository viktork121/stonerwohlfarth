...
! The energy functional and its parameters as well as the minimizer, are found in the sw_minimizer module.
	...
	! gamma values chosen: {1 deg, 45 deg, 90 deg} convert to radians
	 
    gammas = [1D0*PI/180D0, 45D0*PI/180D0, 90D0*PI/180D0] 
	
	! Minimize energy and store values to plot hysteresis
	
    OPEN(unit = 1, file = "output.txt", status = "replace", action = "write")
    WRITE(1, *) "gamma	", "phi_min	", "H_ext"
    DO i = 1, 3
        phi_0 = 0.0D0
        DO j = 1, 200
            ...
            WRITE(1, *) gammas(i), phi_min, H_range(j)
        END DO
    END DO
    ...
    H_values = [0.25D0, 0.5D0, 0.75D0, 1D0, 1.25D0, 1.5D0, 1.75D0, 2D0] ! Field values; dimensionless units
    
    OPEN(unit = 2, file = "output_energies_gamma_0.txt", status = "replace", action = "write") 
    WRITE(2, *) "phi	", "energy	"
    
    DO j = 1, 8
    	DO i = 1, 100
    		energy_single_val = SW_energy(phi_range(i), 0D0, H_values(j)) ! Field with angle gamma = 0
    		WRITE(2, *) phi_range(i)*180/PI, energy_single_val
    	END DO
    END DO
    
    OPEN(unit = 3, file = "output_energies_gamma_45.txt", status = "replace", action = "write")
    WRITE(3, *) "phi	", "energy	"
    
    DO j = 1, 8
    	DO i = 1, 100
    		energy_single_val = SW_energy(phi_range(i), PI/4., H_values(j)) !Field with angle gamma = 45 deg
